...
public class MerchantConsumer extends BaseNotificationConsumer {
    ...
    public void consume (String orderIdAndMerchant){
        LOGGER.info("Consuming {}", orderIdAndMerchant);
        try {
            String[] tokens = orderIdAndMerchant.split("\\.");
            notify(Integer.parseInt(tokens[0]), Integer.parseInt(tokens[1]));
        } catch (Exception e){
            LOGGER.warn("Unable to consume message: {}", orderIdAndMerchant, e);
        }
    }

    private void notify (Integer orderId, Integer merchantId)
